...
class VistaJuego:
    def __init__(self):
        ...
        self.boton_modo_online = Boton("Jugar Online", SCREEN_WIDTH // 2 - 150, y_menu_start + 2 * (menu_button_height + menu_button_spacing), 300, menu_button_height)
    ...
    def _actualizar_visibilidad_botones(self, estado_global_juego):
        ...
        mostrando_online = es_menu_inicio and self.modo_online_activo

        self.boton_inicio.activo = es_menu_inicio and not mostrando_selector and not mostrando_online
        self.boton_abrir_editor.activo = es_menu_inicio and not mostrando_selector and not mostrando_online
        self.boton_cargar_nivel_editado_abrir_selector.activo = es_menu_inicio and not mostrando_selector and not mostrando_online
        self.boton_salir_juego_principal.activo = es_menu_inicio and not mostrando_selector and not mostrando_online
        self.boton_modo_online.activo = es_menu_inicio and not mostrando_selector and not mostrando_online
        ...
        self.boton_crear_sala.activo = mostrando_online
        self.boton_volver_de_online.activo = mostrando_online
    ...
    def _dibujar_menu_online(self):
        ...
        self.boton_crear_sala.dibujar(self.screen)
        self.boton_volver_de_online.dibujar(self.screen)
    ...
    def dibujar_estado_juego(self, estado_del_modelo, estado_global_juego):
        ...
        if estado_global_juego == PAUSA:
            ...
            if self.modo_online_activo:
                self._dibujar_menu_online()
            elif self.mostrando_selector_nivel_editado:
                self._dibujar_selector_nivel_editado()
            else:
                self.boton_inicio.dibujar(self.screen)
                self.boton_modo_online.dibujar(self.screen)
    ...
    def manejar_eventos_ui(self, evento, estado_actual_juego):
        if estado_actual_juego == MENU_INICIO:
            if self.modo_online_activo:
                if self.boton_crear_sala.manejar_evento(evento):
                    return ("unirse_sala", self.input_room_code)
                if self.boton_volver_de_online.manejar_evento(evento):
                    self.modo_online_activo = False
                ...
                if self.boton_modo_online.manejar_evento(evento):
                    self.modo_online_activo = True
                    return "abrir_menu_online"
